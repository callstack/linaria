...
export function run(
  ...
  dirName = dirName.replace(/\/src\/(shaker|extractor)$/g, '/__tests__');
  ...
  const projectDir = dirname(dirName);

  beforeAll(() => {
    Module._resolveFilename = (filename: string, options) => {
      let dir = dirname(options.filename);
      if (dir === join(projectDir, '__tests__')) {
        dir = join(projectDir, 'src', '__tests__');
      } else if (dir === projectDir) {
        dir = join(projectDir, 'src', '__tests__');
      }
      ...
      return require.resolve(filename, resolveOptions);
    };
  });
