...
import { Identifier, Node, Program } from '@babel/types';
...
type ShakerState = State & {
  getTagType: (id: NodePath<Identifier>) => string | null;
};

class GraphBuilder {
  public readonly graph = new DepsGraph();
  public readonly meta = new Map<string, any>();

  static build(root: Node, options: StrictOptions): DepsGraph {
    return new GraphBuilder(root, options).graph;
  }

  constructor(rootNode: Node, options: StrictOptions) {
    traverse<ShakerState>(
      rootNode,
      {
        Program: {
          enter(path: NodePath<Program>, state: ShakerState) {
            const { imports, exports } = collectExportsAndImports(path);
            console.log(imports, exports);

            const usedTags = findImportedTags(babel, path);
            state.getTagType = (id: NodePath<Identifier>) =>
              usedTags.get(id) ?? null;
            console.log('path.scope.bindings', path.scope.hasGlobal('exports'));
          },
        },
      },
      undefined,
